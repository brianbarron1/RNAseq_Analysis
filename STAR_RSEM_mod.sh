...
nThreadsSTAR=$6 # number of threads for STAR, 8 threads have issue..
...
STARparRun=" --runThreadN $nThreadsSTAR --genomeLoad NoSharedMemory  --limitBAMsortRAM 10000000000"
...
str_SE|str_PE)
	  ...
	  # the next line was added by KKY
	  LC_COLLATE=C sort -k1,1 -k2,2n sig.tmp > sig.sort.tmp
          $bedGraphToBigWig sig.sort.tmp  chrNL.txt Signal.$imult.strand${str[istr]}.bw
      done
      done
      ;;
unstr_SE|unstr_PE)
	  ...
	  #the next line was added by KKY
	  LC_COLLATE=C sort -k1,1 -k2,2n sig.tmp > sig.sort.tmp         
 	  $bedGraphToBigWig sig.sort.tmp chrNL.txt  Signal.$imult.unstranded.bw
      done
      ;;
esac
...
#### modified to take care of the memory of the node, KKY
